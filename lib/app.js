...
module.exports.initialize = function(config) {
  ...
  // File Uploads
  var upload = multer({ dest: app.locals.config.get("application").uploadtempdir });
  ...
  app.post('/uploadFile', upload.single('wiki_file'), function (req, res, next) {
    ...
    var whiteList = app.locals.config.get("application").staticWhitelist;
    var patterns = whiteList.split(',');
    var regexpList = [];

    for(var i = 0; i < patterns.length; i++) {
      var pattern = patterns[i].trim();
      var result = /^\/(.*)\/(.*)/.exec(pattern);

      if (result) {
        try {
	      var regexp = RegExp(result[1], result[2]);
    ...
    var rep_file = app.locals.config.get("application").imageSubdir + '/' + file_name;
    var fs_file = app.locals.config.get("application").repository + '/' +
		app.locals.config.get("application").docSubdir + '/' + rep_file;
    ...
    fs.readFile(temp_file, function(err, data) {
	  ...
	  {
          output_msg = 'File size too big. Max 1MB allowed in wiki';
          res.render('upload_status', {message: output_msg});
		  return;
	  }
	  if (!regexpList.some(function(regexp) { return regexp.test(fs_file); })) {
	    output_msg = 'File extension is not allowed. ' +
			 'staticWhitelist parameter is set to:<br/>' + whiteList;
        res.render('upload_status', {message: output_msg});
		return;
	  }
	  
      fs.writeFile(fs_file, data, function(err) {
          var git_msg = req.body.file_message;
          Git.add(rep_file, git_msg, req.user.asGitAuthor, function(err) {
          });
          output_msg = 'File uploaded to: <pre>' + rep_file + '</pre><br/>' +
			'Add it as an image using: <pre>![Alternative text](' + rep_file + ')</pre><br/>' +
			'Add a link to it using: <pre>[Link text](' + rep_file + ')</pre>';
          res.render('upload_status', {message: output_msg});
      });
    });
  });
